/* =========================================================
   ...
   — quem indicou e quem foi indicado — e uma coluna BU que diz de qual
   unidade veio. O backend NÃO precisa de nenhuma alteração para esta página:
   INDICACAO_COLEGIO já está configurado nele.

   ---------------------------------------------------------
   ESTA PÁGINA É IRMÃ DA indicaçõesCPPEM
   ---------------------------------------------------------
   Mesma estrutura, mesmo HTML, mesmo CSS — muda o bloco BU aqui embaixo, os
   tokens de cor no <style> do index.html e o texto dos benefícios. Mantenha
   as duas parecidas: é o que faz uma correção feita numa valer para a outra.
   ========================================================= */
...
const BU = {
  ...
  chave: "INDICACAO_COLEGIO",

  nome: "COLEGIO",

  /* WhatsApp da secretaria, usado no botão da tela de sucesso e no rodapé. */
...
/* Nome de verdade tem pelo menos duas partes. A equipe liga para essa pessoa —
...
function validar() {
  ...
  if (ok) {
    ...
    if (meu === dele) {
      marcarErro(campos.telefoneIndicado, "Este é o seu próprio WhatsApp. Informe o do responsável que você está indicando.");
      ok = false;
    }
  }
...
/* =========================================================
   ...
   vale é o da escola, conferido na confirmação da matrícula — por isso o
   texto nunca promete nível nenhum a partir daqui.
   ========================================================= */

const CHAVE_PLACAR = "colegio_indicacoes_enviadas";
...
/* =========================================================
   ...
   página nunca depende disto para funcionar.
   ========================================================= */
...
(function animarNiveis() {
  ...
  /* Observa o primeiro card e solta os três juntos: observar um a um faria a
     sequência recomeçar no meio da rolagem, e o escalonamento sumiria. */
  aoAparecer(cards[0], () => {
